...
router.post('/creditNoteCreate', async (req, res) => {
  try {
    ...
    let str=`Credit note is for client ${body.clientDetail.client} created and item is ${itmnamearr} and quantity is ${itmqtyarr} recieved `
    let j={companyname:type,itemId:max,actionType:'CREATE',changedBy:"ABDUL",changeDetails:str,model:"Credit"}
    console.log(j)
    let log=new Logs(j) 
    await log.save()
   //here ending
